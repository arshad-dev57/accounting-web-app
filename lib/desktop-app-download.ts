 ...
 *   NEXT_DESKTOP_DOWNLOAD_URL  — fallback / any OS
 *   NEXT_DESKTOP_DOWNLOAD_MAC  — macOS (.dmg / .zip)
 *   NEXT_PUBLIC_DESKTOP_DOWNLOAD_WIN  — Windows (.exe / .msi)
 */
export function getDesktopDownloadUrl(): string | null {
  const fallback = trimEnv(process.env.NEXT_DESKTOP_DOWNLOAD_URL);
  const mac = trimEnv(process.env.NEXT_DESKTOP_DOWNLOAD_MAC);
