...
import { paymentsReceivedService, Payment, Summary, Customer, BankAccount } from '../../api/payments-recieved/route';
...
export default function PaymentsReceivedPage({ searchParams }: { searchParams?: { customerId?: string; invoiceId?: string } }) {
  ...
  const fetchPayments = useCallback(async (resetPage = true) => {
    ...
    try {
      ...
      const response = await paymentsReceivedService.getPayments({
        page,
  ...
  const fetchCustomers = useCallback(async () => {
    try {
      const data = await paymentsReceivedService.getCustomers();
      setCustomers(data || []);
    } catch (error) {
  ...
  const fetchBankAccounts = useCallback(async () => {
    try {
      const accounts = await paymentsReceivedService.getBankAccounts();
  ...
  const fetchSummary = useCallback(async () => {
    try {
      const data = await paymentsReceivedService.getSummary();
      setSummary(data);
    } catch (error) {
  ...
  const fetchUnpaidInvoices = useCallback(async (customerId: string) => {
    try {
      const data = await paymentsReceivedService.getUnpaidInvoices(customerId);
  ...
  const handleRecordPayment = async (data: any) => {
    setSubmitting(true);
    try {
      await paymentsReceivedService.recordPayment(data);
  ...
  const handleClearCheque = async (paymentId: string) => {
    ...
    try {
      await paymentsReceivedService.clearCheque(paymentId);
  ...
  const handleDeletePayment = async (paymentId: string) => {
    ...
    try {
      await paymentsReceivedService.deletePayment(paymentId);
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <button
                ...
                className="flex items-center gap-1 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-green-500 text-white rounded-lg text-xs md:text-sm font-semibold hover:bg-green-600 transition-all shadow-lg shadow-green-500/25"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-4">
              <p className="text-[10px] md:text-xs text-gray-400 font-medium">Total Received</p>
              <p className="text-lg md:text-xl font-bold text-green-600 mt-0.5 md:mt-1">{formatCurrency(summary.totalReceived)}</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-4">
                  ...
                  >
                    <div className="p-3 md:p-4">
                      <div className="flex items-center gap-3 md:gap-4">
                        <div className="p-2 md:p-2.5 rounded-xl bg-green-100">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 md:gap-2 mt-0.5">
                            <span className="text-[10px] md:text-xs text-gray-500">{payment.customerName}</span>
                            <span className="text-[10px] md:text-xs text-gray-300">•</span>
                            <span className="text-[10px] md:text-xs text-gray-400">Invoice: {payment.invoiceNumber}</span>
                            <span className="text-[10px] md:text-xs text-gray-300">•</span>
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-sm md:text-base font-bold text-green-600">{formatCurrency(payment.amount)}</p>
                          <p className="text-[10px] md:text-xs text-gray-400">Received</p>
                        </div>
                      </div>
                    </div>
                  </div>
...
function RecordPaymentForm({
  ...
  const handleCustomerChange = async (customerId: string) => {
    setFormData(prev => ({ ...prev, customerId, invoiceId: '', amount: '' }));
    if (customerId) {
      ...
      <div className="p-4 md:p-6 max-h-[600px] md:max-h-[700px] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {formData.customerId && unpaidInvoices.length === 0 && !isLoadingInvoices && (
              <p className="text-xs text-orange-600 mt-1">No unpaid invoices for this customer</p>
            )}
          </div>
          ...
          {formData.paymentMethod !== 'Cash' && (
            <div>
              <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-1.5">Deposit To *</label>
              <select
              ...
              >
                ...
                {bankAccounts.map((acc: any) => (
                  <option key={acc.id} value={acc.id}>
                    {acc.name} - {acc.accountNumber} (Balance: {formatCurrency(acc.balance)})
                  </option>
                ))}
              </select>
            </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-3 md:p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
        <div className="flex items-start justify-between px-4 md:px-6 py-4 md:py-5 border-b border-gray-100 bg-gradient-to-r from-green-500/5 to-transparent">
        ...
        <div className="p-4 md:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          <div className="grid grid-cols-3 gap-3 md:gap-4 mb-4">
            <div className="bg-green-50 rounded-xl p-3 md:p-4">
              ...
              <p className="text-lg md:text-xl font-bold text-green-600">{formatCurrency(payment.amount)}</p>
            </div>
            <div className="bg-purple-50 rounded-xl p-3 md:p-4">
              <p className="text-[10px] md:text-xs text-gray-500">Invoice</p>
              <p className="text-lg md:text-xl font-bold text-purple-600">{payment.invoiceNumber}</p>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between py-2 border-b border-gray-50">
              ...
              <span className="text-sm font-medium text-gray-800">{payment.customerName}</span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-gray-50">
              <span className="text-xs text-gray-400 font-medium">Invoice Number</span>
              <span className="text-sm font-medium text-gray-800">{payment.invoiceNumber}</span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-gray-50">
              <span className="text-xs text-gray-400 font-medium">Invoice Amount</span>
              <span className="text-sm font-medium text-purple-600">{formatCurrency(payment.invoiceAmount)}</span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-gray-50">
          ...
          <div className="border-t border-gray-100 pt-4 mt-4">
            <div className="flex flex-col sm:flex-row gap-2">
              {payment.paymentMethod === 'Cheque' && payment.status !== 'Cleared' && (
                <button
