        ...
        <>
          ...
          <div className="flex gap-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  {[
                    { label: 'Order Revenue', value: formatCurrency(kpiData.orders?.revenue || 0), icon: ShoppingCart, color: 'indigo' },
                    { label: 'Invoice Total', value: formatCurrency(kpiData.invoices?.grandTotal || 0), icon: Receipt, color: 'teal' },
                    { label: 'Collected', value: formatCurrency(kpiData.invoices?.paidAmount || 0), icon: DollarSign, color: 'green' },
                    { label: 'Outstanding', value: formatCurrency(kpiData.invoices?.outstanding || 0), icon: FileText, color: 'orange' },
                    { label: 'Orders', value: kpiData.orders?.count?.toString() || '0', icon: ShoppingCart, color: 'blue' },
                    { label: 'Invoices', value: kpiData.invoices?.total?.toString() || '0', icon: Receipt, color: 'purple' },
                  ].map((kpi, index) => (
                    <div 
                      key={index}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2 pt-2">
                          <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                            ...
                            {card.data?.salesChangePercent >= 0 ? '+' : ''}{card.data?.salesChangePercent?.toFixed(1) || 0}% sales
                          </span>
                          <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="font-semibold text-gray-800 text-sm">Orders</p>
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="font-semibold text-gray-800 text-sm">Invoices</p>
                  ...
                  <button className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-all flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="font-semibold text-gray-800 text-sm">Sales Return</p>
                  ...
                  <button className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-all flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="font-semibold text-gray-800 text-sm">Refunds</p>
                      <p className="text-xs text-gray-500">Handle refunds</p>
                    </div>
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    {topCustomers.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">No customers data</div>
                    ) : (
                      <div className="space-y-3">
                        {topCustomers.slice(0, 5).map((customer, index) => (
                          <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all">
                            ...
                            <p className="text-sm font-bold text-gray-800">{formatCurrency(customer.totalSpent)}</p>
                          </div>
                        ))}
                      </div>
                    )}
